...
exports.book_delete_get = (req, res, next) => {
  async.parallel(
    ...
    (err, results) => {
      ...
      if (results.book_bookinstances == null) {
        // No results
  ...
  async.parallel(
    ...
    (err, results) => {
      ...
      if (results.book_bookinstances.length > 0) {
        // Book have bookinstance. Render in same way as for GET route.
        res.render("book_delete", {
          title: "Book Delete",
          book: results.book,
          book_bookinstances: results.book_bookinstances,
      ...
      // Book hasn't bookinstance. Delete object and redirect to the list of books.
      Book.findByIdAndDelete(req.body.bookid, (err) => {
        if (err) {
          return next(err);
        }
        // Successes - go to books list
        res.redirect("/catalog/books");
      });
    }
  );
